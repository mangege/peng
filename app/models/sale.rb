...
class Sale < ActiveRecord::Base
  ...
  validates_numericality_of :inlay,:pt,:gold,:kgold, :other,:message => "营业额不能为空且要大于或等于0的数字,支持小数", :greater_than_or_equal_to => 0
  ...
  protected
  def sum_day
    self.day = self.inlay + self.pt + self.gold + self.kgold + self.other
  end
